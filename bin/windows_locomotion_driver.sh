...
## It currently requires Chore.jar to be in the folder where MWT data is stored. 
...
## bash bin/windows_locomotion_driver.sh data results 1 N2 https://webdav.server/location folder_to_backup_to
##
## Example usage of this script from the Bash Shell - without backup to webdav
## bash bin/windows_locomotion_driver.sh data results 1 N2
...
## Lines 39 to 87 of this script check if the given results folder contains any files which
...
for folder in */; do java -Xmx$3g -jar Chore.jar --shadowless -p 0.027 -M 2 -t 20 -S -N all -o fDpesSlLwWaAmMkbPcdxyuvor1234 --plugin Reoutline::despike --plugin Respine --plugin MeasureReversal::all $folder; done
